from argparse import ArgumentParser
from lxml import etree
import re
import signal
import sys
from tqdm import tqdm

def grep(query: str, flags: list[str], pagename = False, fmt = "{}", output = sys.stdout):
    if flags:
        vals = [flagmap[item] for item in flags if item in flagmap]
        flags = 0
        for f in vals:
            flags |= f
    
    # latest.xml should be enwiktionary-YYYYMMDD-pages-meta-current.xml
    context = etree.iterparse("dumps/latest.xml", events=("end",), tag="{*}page")

    # The `pagename` flag allows dynamically including pagename as "{{PAGENAME}}" in
    # your query.
    if not pagename:
        exp = re.compile(query, flags = flags)
    
    if output == sys.stdout:
        iterator = context  # don't use tqdm when writing to stdout, it looks stupid
    else:
        iterator = tqdm(context, unit = "ppg", desc = "Searching", dynamic_ncols = True)

    for event, elem in iterator:
        title = elem.find("{*}title").text
        text = elem.find("{*}revision/{*}text").text

        if pagename:
            dyn = query.replace("{{PAGENAME}}", re.escape(title))
            exp = re.compile(dyn, flags = flags)

        if not exp.search(text if text else ""):
            continue

        print(fmt.format(title), file = output)

        elem.clear()
        while elem.getprevious() is not None:
            del elem.getparent()[0]

if __name__ == "__main__":
    parser = ArgumentParser(
        prog = "dump_grep",
        description = "Script for searching Wikimedia dumps",
    )

    parser.add_argument("query",
        help = "regex (see documentation for the re module)")

    parser.add_argument("-f", "--flags",
        help = "regex flags to apply to the query, e.g. 'mi' for multiline and case-insensitive")

    parser.add_argument("--pagename", action = "store_true",
        help = "if passed, {{PAGENAME}} in the query is dynamically substituted for the pagename for each page")

    parser.add_argument("--format",
        help = "format to output pages in, e.g. '* [[{}]]' to print titles out like a wikitext list")

    parser.add_argument("-o", "--output",
        help = "output file (defaults to stdout)")
    
    args = parser.parse_args()

    flagmap = {
        "a": re.ASCII,
        "i": re.IGNORECASE,
        "l": re.LOCALE,
        "m": re.MULTILINE,
        "s": re.DOTALL,
        "u": re.UNICODE,
        "x": re.VERBOSE,
    }

    args.flags = list(args.flags.lower()) if args.flags else ""

    if args.output:
        with open(args.output, "w", encoding = "utf-8") as file:
            grep(args.query, args.flags, pagename = args.pagename, fmt = args.format, output = file)
    else:
        grep(args.query, args.flags, pagename = args.pagename, fmt = args.format)
